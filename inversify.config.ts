...
export const firebaseReferences = new ContainerModule((bind: interfaces.Bind, unbind: interfaces.Unbind) => {
	...
	const contentIdMapIdMapRef = firebase.database().ref(FIREBASE_PATHS.CONTENT_ID_MAP_ID_MAP);
...
export const loaders = new ContainerModule((bind: interfaces.Bind, unbind: interfaces.Unbind) => {
	...
	const {ContentIdMapIdMap, ContentIdMapIdMapArgs} = require('./app/objects/contentIdMapIdMap/contentIdMapIdMap');
	myContainer.bind(TYPES.IContentIdMapIdMap).to(ContentIdMapIdMap);
	myContainer.bind(TYPES.ContentIdMapIdMapArgs).to(ContentIdMapIdMapArgs);
});
...
const rendering = new ContainerModule((bind: interfaces.Bind, unbind: interfaces.Unbind) => {
	...
	bind<FGetStore>(TYPES.fGetStore).toConstantValue(() => {
		return {
			commit() {
			}
		} as any as Store<any>
