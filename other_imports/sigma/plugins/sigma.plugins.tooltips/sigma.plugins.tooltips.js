		...
		for (var i = 0; i < _nodeTooltips.length; i++) {
			...
			if (_nodeTooltips[i].position !== undefined) {
				if (_nodeTooltips[i].position !== 'top' &&
					...
					_nodeTooltips[i].position !== 'card-center' &&
					_nodeTooltips[i].position !== 'css') {
					throw new Error('"options.position" is not "top", "bottom", "left", "right","center", "card-center", or "css", was ' + _nodeTooltips[i].position);
				}
			}
