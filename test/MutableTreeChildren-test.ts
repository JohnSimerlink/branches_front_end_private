...
import {MutableTreeChildren} from '../app/objects/tree/MutableTreeChildren';
import {TreeMutationTypes} from '../app/objects/tree/TreeMutationTypes';

describe('MutableTreeChildren', () => {
    ...
    const treeChildren = new MutableTreeChildren()
    ...
    it('INIT should have no children after constructor', () => {
        expect(treeChildren.getIds()).to.deep.equal(INIT_CHILDREN_VALUE)
    })

    it('INIT should return history of mutations on the point after creation', () => {
        expect(treeChildren.mutations().length).to.equal(0)
    ...
    it('GOOD ADD_CHILD mutation should add childId to children array and add entry to mutation history', () => {
        ...
        expect(treeChildren.getIds()).to.deep.equal(FIRST_CHILDREN_VALUE)
        expect(treeChildren.mutations().length).to.equal(1)
    })
    ...
    and keep the data and mutation values the same`, () => {
        ...
        expect(() => treeChildren.addMutation(disallowedRedundantMutation)).to.throw(RangeError)
        expect(treeChildren.getIds()).to.deep.equal(FIRST_CHILDREN_VALUE)
        expect(treeChildren.mutations().length).to.equal(1)
    })
    ...
    and should keep data and mutations the same`
        , () => {
            ...
            expect(() => {treeChildren.addMutation(badRemoveMutation)}).to.throw(RangeError)
            expect(treeChildren.getIds()).to.deep.equal(FIRST_CHILDREN_VALUE)
            expect(treeChildren.mutations().length).to.equal(1)
        })
    ...
    and should add another mutation`, () => {
        ...
        expect(treeChildren.getIds()).to.deep.equal(INIT_CHILDREN_VALUE)
        expect(treeChildren.mutations().length).to.equal(2)
    })
    ...
    and should increment mutations length`, () => {
        ...
        expect(treeChildren.getIds()).to.deep.equal(FIRST_CHILDREN_VALUE)
        expect(treeChildren.mutations().length).to.equal(3)
    })
    ...
    and num mutations to 6`, () => {
        ...
        expect(treeChildren.getIds()).to.deep.equal(childrenAfterSixthSuccessfulMutation)
        expect(treeChildren.mutations().length).to.equal(6)
    })
    ...
    and num mutations to 7`, () => {
        ...
        expect(treeChildren.getIds()).to.deep.equal(childrenAfterSeventhSuccessfulMutation)
        expect(treeChildren.mutations().length).to.equal(7)
    })
