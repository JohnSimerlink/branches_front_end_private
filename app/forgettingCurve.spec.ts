...
test('should return 10 dbE for 1/e sampleContentUser1Proficiency and 10 seconds', () => {
    expect(curve.measurePreviousStrength(
        {estimatedPreviousStrength: 39, R: 100 / curve.e, t: 10})).to.closeTo(10, .1);
});
test('should return 50 dbE for 75% sampleContentUser1Proficiency and 8 hours', () => {
    expect(curve.measurePreviousStrength(
        {estimatedPreviousStrength: 39, R: 100 * .75, t: 8 * 60 * 60})).to.closeTo(50, .1);
});

/**
 * Estimating previous strength tests.
 */

test('should return previous strength for 1/e sampleContentUser1Proficiency', () => {
    expect(curve.estimateCurrentStrength(
        {
            previousInteractionStrengthDecibels: 39, currentProficiency: 100 / curve.e,
            secondsSinceLastInteraction: 8 * 60 * 60
        })).to.closeTo(39, .1);
});
test('should return previous strength for 0 seconds since last review', () => {
    expect(curve.estimateCurrentStrength(
        {
            previousInteractionStrengthDecibels: 39, currentProficiency: 100 / curve.e,
...
test('should return + 10(e-1) dB for 100% sampleContentUser1Proficiency and longgg time since last review', () => {
...
test('should return + 10(1 - 1/e) dB for 100% sampleContentUser1Proficiency at Tc', () => {
    const originalStrength = 39;
    ...
    expect(curve.estimateCurrentStrength(
        {
            previousInteractionStrengthDecibels: 39,
            currentProficiency: 99.99999, secondsSinceLastInteraction: Tc
        })).to.closeTo(newExpectedStrength, .1);
});
test('should return + 4.4dB for 99% sampleContentUser1Proficiency at 40 minutes past a 39 strength memory ', () => {
    const originalStrength = 39;
