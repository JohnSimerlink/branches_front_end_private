...
CMD cd /frontend && echo "Running npm install" && npm install && \
  ...
  NODE_ENV=development UV_THREADPOOL_SIZE=128 ./node_modules/.bin/webpack-dev-server --display-reasons --display-chunks --loglevel verbose --progress --inline --hot --host 0.0.0.0; \
  fi
